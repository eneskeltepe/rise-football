...
2026-06-12 **özellik dalgası: Duran top sistemi (v2.13.0)**: Yeni modül `49-setpieces.js` (48-awards'tan sonra). (1) **Penaltıcı/Frikikçi görevi (A4)**: `updateSetPieceDuty` (advanceWeek'ten haftalık) — `attrs.penalti≥70`/`serbestVurus≥72` + güven≥55 → görev; histerezisli geri alma; kaleci hariç; değişimde toast; profilde rozet (`p.setPieceDuty`). Canlı maçta `maybeSetPieceMoment` (ticker hook'u, 45-matchengine): görevli kullanıcı penaltı (sert/plase/panenka) ve frikik (direkt/kavisli/ortala) kullanır, kaleci kullanıcı rakip penaltısını kurtarmaya çalışır; gösterilen % = gerçek şans (`_penKickChance`/`_fkGoalChance`/`_gkPenSaveChance`); per-maç tavan sayaçları (`penMomentsUsed`/`fkMomentsUsed`/`oppPenMomentsUsed`) startMatchDay'de SIFIRLANIR (activeMatch kalıcı nesne dersi). (2) **Kupa uzatma + seri penaltı (A5)**: turu bitiren maçta eşitlik (`_cupTieDeciding`) → `_etSim` uzatması + hâlâ eşitse seri penaltı; kullanıcı sahada bitirdiyse `_interactiveShootout` (overlay; köşe seç / kaleciysen her atışta tahmin; 5+5 seri ölüm; her atıştan sonra İKİ YÖNLÜ matematiksel erken bitiş; `ctx.rng` enjekte edilebilir — testte arka plan Math.random tüketicilerinden etkilenmez), değilse `cupTieBreakSync`; `endEuroMatch` beraberlik yolu ASENKRON (`_finishCup` devamı), `simEuroMatch` senkron sürümü kullanır (autoSim döngüsü bozulmaz); `_recordEuro` `cur.penShootout` sonucunu kullanır (yoksa eski zar geri-uyum), `rd.penScore` özet/eleme listesi/toast'ta görünür. Test: `test_setpieces`(24); süit 646/646 (61 dosya).

2026-06-12 **özellik dalgası: Rekorlar + Altın Top + Ayın Oyuncusu (v2.12.0)**: Yeni modül `48-awards.js` (47-worldstats'tan sonra yüklenir). (1) **Rekorlar (A1)**: `computeWorldRecords(slot)` — WorldDB `playerSeasons` tek geçiş (cursor) → tüm-zamanlar kariyer gol/asist/maç/CS + tek-sezon gol/asist top-10; kullanıcı `careerStats`/`seasonHistory` satırlarıyla birleşir (kullanıcı playerSeasons'a yazılmadığı için çift sayma imkânsız); isimler WorldDB `players` store'undan (regen dahil); sezon başına cache. (2) **Altın Top (A2)**: `computeBallonDor(slot, season)` — 94-bindings sezon-devri zincirinde agregat+özet SONRASI; skor `g×2 + a×1.4 + cs×0.5 + maç×0.05`, lig ağırlığı `ortGüç/80` (0.75–1.1), şampiyonluk +8 (özet meta'sından); top-10 + kullanıcı sırası `gameState.ballonHistory`'ye (çift-kayıt/çift-kupa korumalı); kullanıcı kazanırsa "Altın Top" kupası + fansLove. (3) **Ayın Oyuncusu (A3)**: `maybeMonthlyAward` (advanceWeek'ten) — her 4 haftada son ayın LİG maçları (≥3 maç, ≥30dk, ort ≥7.5) → `p.monthlyAwards` rozeti + fansLove+3; `_lastMonthlyKey` ile ay başına tek değerlendirme. UI: İstatistikler sekmesinde görünüm değiştirici (Krallıklar/Rekorlar/Altın Top — `statsView.view`), kullanıcı profili kariyer özetinde Ayın Oyuncusu sayacı. Test: `test_awards`(20); süit 622/622 (60 dosya).

2026-06-12 **düşük-öncelik temizlik + teklif rolü dalgası (v2.11.0)**: (1) **Teklif kadro rolü (kullanıcı bildirimi)**: `calculateRealisticSquadRole` genç kapısı (yaş≤19 + kulüp gücü>75) KOŞULSUZDU → yıldızlaşan gence bile tüm teklifler "Altyapı / Rotasyon" geliyordu; artık yalnız oyuncu kulüp seviyesinin altındaysa (fark>2) uygulanır. (2) **Düşük buglar (D1–D9)**: yüklemede `fixtureViewingWeek` güncel haftaya senkron; `SLOT_COUNT=10`; bozuk İngiltere bayrak literali düzeltildi; `ageGenFillers` (52-market, sezon devrinde 94-bindings çağırır) — dolgu oyuncular yaşlanır, 34+ emekli olur yerine genç gelir; karar-anı gol/asist `shots`+`shotsOnTarget` birlikte artar (isabetli>toplam bug'ı); ölü kod silindi (`weeksAtCurrentClub`, `SLOT_N`, `lids`); `_doSub` giren yedeğe mevki-yetkinlik çarpanı uygular (efektif OVR, FAZ C ile tutarlı); karar-anı başarısızlığında yenen golün golcüsü gerçek rakip oyuncu (takım adı değildi); kariyer-hafta hesaplarında kalan 2026 hardcode'lar → `START_SEASON`. (3) **Teknik borç #22**: `test_reloadseason` — GERÇEK sezon-devri yolu (btn-start-next-season) + sezon-2'de reload sonrası lig üyeliği/güç/puan durumu/skor birebir karşılaştırması. Testler: `test_lowfixes`(17) `test_reloadseason`(12) `test_offerrole`(8); süit 602/602 (59 dosya).

2026-06-11 **orta-öncelik + sezon-içi güç dalgası (v2.10.0)**: (1) **"Güç sezon ortasında sabit" kuralı KALDIRILDI** (kullanıcı isteği): `applyTransferPowerDelta(buyer,seller,ovr)` (52-market) — yıldız alan kulüp AYNI sezon güçlenir (fark×0.08, tavan 1.5; kulüp seviyesi altındaki oyuncu etkisiz), satan zayıflar; pencere-içi/sezon-sonu AI piyasası + haber yolu + kullanıcı transferi bundan geçer (kiralık nötr). Kalıcılık: `gameState.teamPowerDelta` → `restoreWorldState` replay üstüne uygular. Tutarlılık: geçmiş haftaların diğer-lig skorları WorldDB'deki SAKLI sonuçtan gösterilir (60-ui `renderFixturesForWeek` async patch); euro lig-fazı skorları kampanyada sabitlenir (`e._lpScores`). (2) **Finans simetrisi**: `_estWages` kullanıcı-kulübü özel yolu (gerçek maaş×0.9 vs formül×0.6 → hep daha fakir) kaldırıldı; herkes aynı formül. (3) **Kupa-bağlılık (O2)**: sezon ortası transferde bu sezon kupada OYNADIYSAN kampanya yeni kulüple retroaktif kurulmaz (`_euroCupTied`); hiç oynamadıysan serbest. (4) **Maç içi**: `_slotPrimaryFam` ('Bek'→FB,'Kanat'→W; değişiklik AI'ı artık bek/kanatı doğru sınıflar), kullanıcı KIRMIZI KART (ikinci sarı kesin + sert faulde ~%8 direkt; atılan oyuncunun yerine kimse giremez), değişiklik hakkı 0 guard'ı (`_subInForUser` 10 kişi, sayaç negatif olmaz). (5) **Dünya kaydı**: oynamadığın haftalar (sakat/cezalı/kadro dışı) `_recordUserMatchToWorld(...,{ignoreState:true})` ile yazılır; yazımlar `window._worldWriteSync` zincirinde — sezon-sonu agregat bekler (yarış fix'i). (6) **Pencere-bazlı teklif** (O5): maç dönüşü teklifleri sabit hafta 18/36 yerine açık pencerede pencere-başına-bir-kez. (7) **Yaş/OVR gösterim tutarlılığı** (O3): kadro modalı + profil isYouth/isRegen çifte yaş fix'i; arama/kadro önizleme yaş-düzeltilmiş OVR/yaş gösterir. (AÇIK İŞ — O4: WorldDB kalıcı evrim vs `ageAdjustedOvr` çifte yaşlanma modeli — dünya-stat motoru işiyle birlikte tek kaynağa bağlanacak.) Testler: `test_powermid`(10) `test_midmatch`(11) `test_eurotied`(8) `test_worldwrite`(7) `test_offerwindow`(7); süit 565/565 (56 dosya).

2026-06-11 **yüksek-öncelik bug dalgası (v2.9.0)**: (1) `simulateMatchInstantly` sakat/cezalıyken oyuncuyu OYNATMAZ — 90-main sarmalayıcı yoluna (`startMatchDay`) yönlenir: takım maçı oyuncusuz sonuçlanır, ceza düşer (eskiden hiçbir kontrol yoktu). (2) Sözleşme sayaçları KARİYER-TOPLAM hafta birimine geçti (`lastContractRenewalWeek`/`negotiationBlockUntil`; 54-negotiation + 60-ui acceptTransferOffer + 94-bindings yenileme) — sezon devrinde negatif fark/tüm-sezon blok bug'ları bitti; eski kayıtlarda küçük değerler toplam-hafta karşılaştırmasında zararsız (erken izin). (3) Canlı kupa maçında enerji çift düşmez: `_applyPlayerCupOutcome(ps,quiet,{live,playedMins})` canlıda yalnız hızlı-simlenen dakikalar için düşüş; kupada `neverPlayed` guard'ı (yedek hiç girmediyse stat/reyting/güven işlenmez, +12 dinlenme, matchLog'a dnp). (4) `loadCareerSlot` tohumlanmamış slotu (`WorldDB.isSeeded`) arka planda tohumlar (import/temiz IDB → ölü dünya fix'i); `importSaveFromFile` hedef slotun eski dünya verisini `clearSlot` ile temizler. (5) `runSeasonCups` oyuncunun turnuvasının şampiyonunu/akıbetini KAMPANYADAN alır, oyuncu kulübünü diğer kıta kupalarına sokmaz (`simulateCup(comp, excludeTeamId)`), bitmemiş kampanyayı `autoSimDueEuro(99999)` ile tamamlar. Testler: `test_simunavail`(12) `test_negweeks`(7) `test_cupenergy`(11) `test_worldseed`(7) `test_cupchamp`(10); süit 522/522 (51 dosya).
