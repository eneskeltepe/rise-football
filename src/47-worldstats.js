...
(function () {
    ...
    function _rebuild(slot, season) {
        ...
        function apps(ids, isStart, team, lg) { if (!ids) return; for (const pid of ids) { const r = A(pid); r.m++; if (isStart) r.starts++; else r.subApps++; r.team = team; r.lg = lg; } }
        return WorldDB.iterateByIndex('matches', 'bySlotSeason', IDBKeyRange.only([slot, season]), (m) => {
            ...
            if (m.sa === 0 && m.homeXI && m.homeXI.length) A(m.homeXI[0]).cs++;
            if (m.sh === 0 && m.awayXI && m.awayXI.length) A(m.awayXI[0]).cs++;
            ...
            for (const ev of (m.events || [])) {
                ...
                if (ev.type === 'goal') {
                    ...
                    if (ev.assistId != null) { A(ev.assistId).a++; mc[ev.assistId] = (mc[ev.assistId] || 0) + 1; }
                } else if (ev.type === 'yellow') r.y++;
